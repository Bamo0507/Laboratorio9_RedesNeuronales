...
SmartStay Advisors busca apoyar a clientes corporativos y particulares en la selección de propiedades de Airbnb, optimizando precio, calidad y disponibilidad. Además, la empresa tiene un incentivo estratégico adicional: aumentar la ocupación de propiedades con bajo desempeño. Para responder a este objetivo, durante la consultoría se evaluaron modelos de minería de datos capaces de segmentar propiedades por nivel de precio y de estimar el precio por noche a partir de las características disponibles en el conjunto de datos.

El análisis general muestra que los modelos basados en árboles fueron los más adecuados para este problema. En particular, **Random Forest** fue el algoritmo con mejor desempeño tanto para clasificación como para regresión. Esto se debe a que puede capturar relaciones no lineales y combinaciones complejas entre variables, algo esperable en datos de Airbnb, donde el precio y la categoría de una propiedad dependen simultáneamente de múltiples factores como ubicación, disponibilidad, capacidad, reputación, reglas de estancia y comportamiento histórico.

### Preparación y exploración de datos

El conjunto de datos fue depurado eliminando variables identificadoras, enlaces, campos textuales extensos y columnas que no aportaban directamente al modelado o que podían introducir ruido. Luego, se transformó el precio a formato numérico y se conservaron las observaciones con precio disponible. Para mantener una comparación válida entre modelos, se usó la misma partición de entrenamiento y prueba durante las distintas entregas. Este punto es fundamental porque comparar algoritmos con particiones distintas puede producir conclusiones engañosas.

También se construyó una variable categórica de precio con tres niveles: propiedades económicas, intermedias y caras. Esta segmentación permitió abordar el problema desde dos perspectivas: clasificación, para saber en qué categoría se ubica una propiedad, y regresión, para estimar directamente el precio por noche.

### Modelos de clasificación

En clasificación se evaluaron árboles de decisión, Random Forest, Naive Bayes, KNN, regresión logística, SVM y redes neuronales artificiales. Las RNA se probaron con diferentes topologías y funciones de activación. El mejor resultado de RNA fue competitivo frente a algunos modelos clásicos, pero no logró superar el desempeño de Random Forest.

El mayor reto de clasificación estuvo en las propiedades intermedias. Esto tiene sentido porque las propiedades de precio medio suelen compartir características con los extremos: algunas se parecen a propiedades económicas por tamaño o disponibilidad, mientras que otras se parecen a propiedades caras por ubicación, reputación o amenidades. Por esta razón, los errores más frecuentes se concentraron en esa clase.

### Modelos de regresión

En regresión se evaluaron modelos orientados a predecir el precio por noche. Nuevamente, Random Forest fue el modelo con mejor desempeño, al obtener el menor error y el mejor R2. La RNA de regresión presentó resultados débiles: sus errores fueron altos y su capacidad explicativa fue baja. Aunque se realizó tuneo de parámetros, la mejora no fue suficiente para justificar su uso como modelo principal.

Para el negocio, esta diferencia es relevante. Un modelo con errores promedio altos puede llevar a recomendaciones de precio poco realistas, lo cual afecta la competitividad de las propiedades y la confianza de los clientes. Por eso, aunque las redes neuronales son modelos potentes en muchos contextos, en este conjunto de datos no fueron la mejor alternativa.

### Recomendación para negocio

Para SmartStay Advisors se recomienda utilizar **Random Forest como modelo principal de clasificación** para segmentar propiedades en económicas, intermedias y caras. Esta segmentación puede apoyar decisiones rápidas de posicionamiento comercial, selección de propiedades para clientes y detección de alojamientos que podrían necesitar ajustes de precio.

También se recomienda utilizar **Random Forest como modelo principal de regresión** para estimar precios por noche. Este modelo puede apoyar estrategias de pricing competitivo, especialmente cuando se combine con reglas de negocio, revisión de disponibilidad y análisis de ocupación. Si la empresa necesita mayor interpretabilidad para explicar cada decisión a clientes o directivos, el árbol de regresión puede funcionar como modelo complementario, aunque con menor precisión.

### Conclusiones finales

La primera conclusión es que **Random Forest fue el mejor modelo general para clasificación**. Aunque presenta señales de sobreajuste moderado, su rendimiento en prueba fue superior al de los demás algoritmos. Por ello, se recomienda usarlo con validación cruzada y control de complejidad antes de implementarlo en un entorno productivo.

La segunda conclusión es que **Random Forest también fue el mejor modelo para predecir precios**. Su menor RMSE, menor MAE documentado y mayor R2 lo convierten en la opción más sólida para estimar el precio por noche de las propiedades. Este resultado confirma que los modelos basados en ensambles de árboles se adaptan mejor a la estructura del conjunto de datos que los modelos lineales o las RNA probadas.

La tercera conclusión es que **las redes neuronales no fueron la mejor opción para este conjunto de datos**. En clasificación lograron resultados aceptables, pero no superiores; en regresión tuvieron un desempeño bajo. Esto demuestra que un modelo más complejo no siempre produce mejores resultados. Para este caso, la estructura de los datos favorece algoritmos capaces de manejar relaciones no lineales de manera más estable y con menor sensibilidad al escalamiento, como Random Forest.

La cuarta conclusión es que **el análisis de sobreajuste debe formar parte de la decisión final**. Un modelo no debe elegirse solo por tener la métrica más alta, sino por mantener un balance razonable entre desempeño, estabilidad e interpretabilidad. En este proyecto, Random Forest ofrece el mejor desempeño, pero debe monitorearse para evitar exceso de ajuste. Por tanto, la recomendación final es usar Random Forest como modelo principal, acompañado de validación cruzada, revisión periódica de métricas y actualización con nuevos datos de mercado.
